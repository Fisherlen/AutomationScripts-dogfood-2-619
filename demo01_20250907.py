...
"""
新功能说明：
已将广告采集的核心功能分离到 baidu_ad_collector.py 文件中
本文件保留原始测试逻辑，可通过导入模块的方式使用新功能
"""

# 导入新的广告采集模块
from baidu_ad_collector import BaiduAdCollector, print_ad_summary

def test_baidu_with_new_module():
    """使用新模块测试百度广告采集"""
...
def test_baidu_original():
    ...
    try:
        ...
        print("输入搜索关键词'手机'...")
        # 直接访问搜索结果页面
        driver.get("https://www.baidu.com/s?wd=手机")
        time.sleep(5)
        ...
        for indicator in ad_indicators:
            try:
                ad_container = indicator.find_element(
                    By.XPATH, 
                    "./ancestor::*[contains(@class, 'c-container') or contains(@class, 'result') or contains(@class, 'ad')]"
                )
...
if __name__ == "__main__":
    ...
    print("使用新功能模块运行广告采集...")
    print("="*60)
    test_baidu_with_new_module()
    
    # 如需使用原始方法测试，可注释上面一行，取消下面一行的注释
    # test_baidu_original()
